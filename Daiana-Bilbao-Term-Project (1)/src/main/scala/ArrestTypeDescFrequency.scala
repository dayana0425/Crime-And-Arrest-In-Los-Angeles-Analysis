...
object ArrestTypeDescFrequency {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val newDF = crimeDataFrame.na.fill("NO CHARGE DESC")
    val sortedDF = newDF.groupBy("Charge Description").count().orderBy(desc("count")).limit(11)
    sortedDF.select("Charge Description", "count").write.format("csv").save(args(1))
  }
}