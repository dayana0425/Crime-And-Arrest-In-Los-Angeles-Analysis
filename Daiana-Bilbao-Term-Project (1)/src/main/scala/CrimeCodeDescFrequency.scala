...
object CrimeCodeDescFrequency {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val crimeDataFrame = spark.read
                              ...
                              .option("charset", "UTF8")
                              .load(args(0))
    val newDF = crimeDataFrame.na.fill("NO CRIME CODE DESC")
    val sortedDF = newDF.groupBy("Crime Code Description").count().orderBy(desc("count")).limit(10)
    sortedDF.select("Crime Code Description", "count").write.format("csv").save(args(1))
  }
}